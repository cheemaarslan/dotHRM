...
if (!function_exists('getUser')) {
    function getUser()
    {
        ...
        if ($autheUser->hasRole('superadmin')) {
            ...
            $getCompanyId = getCompanyId($autheUser->created_by);
            $company = User::where('id', $getCompanyId)->first();
...
if (!function_exists('getMediaUserIds')) {
    function getMediaUserIds()
    {
        $user = Auth::user();

        // SaaS superadmin — collect every company and their users
        if (isSaas() && $user->hasRole('superadmin')) {
            $allIds = [];
            ...
            foreach ($companies as $companyId) {
                $allIds[] = $companyId;
                getAllCompanyUsers($companyId, $allIds);
            }
            $allIds = array_merge($allIds, [$user->id]);
            return array_unique($allIds);
        }

        // Non-SaaS or SaaS company/employee — same as existing helper
