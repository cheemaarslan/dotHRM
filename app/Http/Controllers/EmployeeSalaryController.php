...
class EmployeeSalaryController extends Controller
{
    public function index(Request $request)
    {
        if (Auth::user()->can('manage-employee-salaries')) {
            ...
            $query = EmployeeSalary::with(['employee.employee', 'creator'])->where(function ($q) {
                if (Auth::user()->can('manage-any-employee-salaries')) {
            ...
            $employeeSalaries->getCollection()->transform(function ($salary) {
                ...
                if ($salary->employee) {
                    ...
                    $salary->employee->employee_id = $salary->employee->employee?->employee_id ?? '';
                }
                return $salary;
            });
    ...
    private function getPayrollCalculationData($employeeSalary, $payrollRun)
    {
        ...
        $attendanceRecords = AttendanceRecord::where('employee_id', $employeeSalary->employee_id)
            ->whereBetween('date', [$payrollRun->pay_period_start, $payrollRun->pay_period_end])
            ->orderBy('date')
            ->get()
            ->map(function ($record) use ($employeeSalary) {
                if ($record->status === 'on_leave') {
                    $leave = LeaveApplication::where('employee_id', $employeeSalary->employee_id)
                        ->where('status', 'approved')
                        ->whereDate('start_date', '<=', $record->date)
                        ->whereDate('end_date', '>=', $record->date)
                        ->with('leaveType')
                        ->first();
                    $record->leave_type_name = $leave?->leaveType?->name ?? null;
                    $record->is_paid_leave   = $leave?->leaveType?->is_paid ?? null;
                }
                return $record;
            });
        ...
        $attendanceSummary = [
            ...
            'full_present_days'   => $payrollEntry->full_present_days,
        ...
        $payrollCalculation = [
            ...
            'lop_days'                => $payrollEntry->lop_days ?? 0,
            'lop_deduction'           => $payrollEntry->lop_deduction ?? 0,
            'unpaid_leave_deduction'  => $payrollEntry->unpaid_leave_deduction ?? 0,
            'effective_paid_days'     => $payrollEntry->effective_paid_days ?? 0,
            'component_earnings'      => $payrollEntry->component_earnings ?? 0,
        ];
