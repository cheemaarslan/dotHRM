...
export default function EmployeeSalaries() {
  ...
  const { auth, employeeSalaries, employees, salaryComponents, filters: pageFilters = {}, flash, globalSettings } = usePage().props as any;
  ...
  const handleFormSubmit = (formData: any) => {
    if (formMode === 'create') {
      if (!globalSettings?.is_demo) toast.loading(t('Creating employee salary...'));
      ...
      if (!globalSettings?.is_demo) toast.loading(t('Updating employee salary...'));
  ...
  const handleDeleteConfirm = () => {
    if (!globalSettings?.is_demo) toast.loading(t('Deleting employee salary...'));
  ...
  const handleToggleStatus = (salary: any) => {
    ...
    if (!globalSettings?.is_demo) toast.loading(`${newStatus === 'active' ? t('Activating') : t('Deactivating')} employee salary...`);
      ...
      render: (value: any, row: any) => (
        <div className="flex items-center space-x-3">
          <div className="relative">
            ...
            <div className={`avatar-fallback w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center text-gray-700 font-semibold text-xs${row.employee?.avatar ? ' hidden' : ''}`}>
              {row.employee?.name?.charAt(0)?.toUpperCase() || 'U'}
            </div>
          </div>
          <div className="flex flex-col">
            <span className="font-medium">{row.employee?.name || '-'}</span>
            {row.employee?.employee_id && (
              <span className="text-xs text-gray-500">{row.employee.employee_id}</span>
            )}
          </div>
        </div>
      )
  ...
  return (
    ...
    >
      ...
      <CrudFormModal
        ...
        title={formMode === 'create' ? t('Setup Employee Salary') : t('Edit Employee Salary')}
        mode={formMode}
      />
