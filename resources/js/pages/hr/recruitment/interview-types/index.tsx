...
import { MessageSquare, Edit, Trash2, Search, X, ChevronDown, ChevronUp, Lock } from 'lucide-react';
...
export default function InterviewTypes() {
  ...
  const { auth, interviewTypes, filters: pageFilters = {}, globalSettings } = usePage().props as any;
  ...
  const canCreate = hasPermission(permissions, 'create-interview-types');
  const canEdit   = hasPermission(permissions, 'edit-interview-types');
  const canDelete = hasPermission(permissions, 'delete-interview-types');
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    if (formMode === 'create') {
      if (!globalSettings?.is_demo) toast.loading(t('Creating interview type...'));
      ...
      if (!globalSettings?.is_demo) toast.loading(t('Updating interview type...'));
  ...
  const handleDeleteConfirm = () => {
    if (!globalSettings?.is_demo) toast.loading(t('Deleting interview type...'));
    router.delete(route('hr.recruitment.interview-types.destroy', currentItem.id), {
      ...
      onError: (errors) => {
        ...
        toast.error(typeof errors === 'string'
          ? t(errors)
          : t('Failed to delete interview type: {{errors}}', { errors: Object.values(errors).join(', ') }));
      },
    });
  };
  ...
  const handleToggleStatus = (item: any) => {
    ...
    if (!globalSettings?.is_demo) {
      toast.loading(newStatus === 'active' ? t('Activating interview type...') : t('Deactivating interview type...'));
    }
    router.put(route('hr.recruitment.interview-types.toggle-status', item.id), {}, {
      onSuccess: (page) => {
        if (!globalSettings?.is_demo) toast.dismiss();
        if (page.props.flash.success) toast.success(t(page.props.flash.success));
        else if (page.props.flash.error) toast.error(t(page.props.flash.error));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 sticky top-4">
            <div className="p-6 border-b border-gray-200 dark:border-gray-700">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                {formMode === 'create' ? t('Add New Interview Type') : t('Edit Interview Type')}
              </h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                {formMode === 'create'
                  ? t('Fill in the details to create a new interview type')
                  : t('Update the interview type details below')}
              </p>
            </div>
            ...
            <form onSubmit={handleFormSubmit} className="p-6 space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="name" className="required" required>{t('Interview Type Name')}</Label>
                <Input
                  id="name"
                  type="text"
                  value={formData.name}
                  onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                  placeholder={t('e.g., Technical, HR, Panel, Phone Screen')}
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder={t('Brief description of the interview type')}
                  rows={3}
              ...
              {/* Status */}
              <div className="space-y-2">
                <Label htmlFor="status" className="required" required>{t('Status')}</Label>
                <Select
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                {(canCreate || canEdit) && (
                  <Button type="submit" className="flex-1">
                    {formMode === 'create' ? t('Add Interview Type') : t('Update Interview Type')}
                  </Button>
                )}
                {formMode === 'edit' && (
        ...
        <div className="lg:col-span-2 space-y-4">
            ...
            <div className="space-y-4">
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <Input
                    type="text"
                    placeholder={t('Search interview types...')}
                    value={searchTerm}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
            {(interviewTypes?.data || []).length > 0 ? (
              <>
                <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t('Interview Types')}</h3>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    {t('Manage interview types used in the recruitment process')}
                  </p>
                </div>
                ...
                <div className="hidden lg:block">
                  <div className="overflow-x-auto">
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                      {(interviewTypes?.data || []).map((item: any) => (
                        <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">

                          <td className="px-4 py-4">
                            <div className="flex items-center">
                              <div className="flex-shrink-0 h-10 w-10 rounded-lg flex items-center justify-center bg-purple-100 text-purple-600 dark:bg-purple-900/30 dark:text-purple-400">
                ...
                {/* Pagination */}
                {interviewTypes?.total > (interviewTypes?.per_page || 10) && (
              ...
              <div className="p-12 text-center">
                ...
                <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
                  {t('No interview types found')}
                </h3>
                <p className="text-gray-500 dark:text-gray-400 mb-6 max-w-sm mx-auto">
                  {isFilterActive
                    ? t('No interview types match your search criteria. Try adjusting your filters.')
                    : t('Create interview types to categorize your recruitment interviews.')}
                </p>
                {!isFilterActive && canCreate && (
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {t('Use the form on the left to add your first interview type.')}
                  </p>
                )}
              </div>
            )}
