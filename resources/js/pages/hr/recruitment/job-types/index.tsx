...
import { Layers, Edit, Trash2, Search, X, ChevronDown, ChevronUp, Lock } from 'lucide-react';
...
export default function JobTypes() {
  ...
  const { auth, jobTypes, filters: pageFilters = {}, globalSettings } = usePage().props as any;
  ...
  const canCreate = hasPermission(permissions, 'create-job-types');
  const canEdit   = hasPermission(permissions, 'edit-job-types');
  const canDelete = hasPermission(permissions, 'delete-job-types');
  ...
  const validate = (): boolean => {
    ...
    if (!formData.name.trim()) {
      errors.name = t('Job Type required');
  ...
  const applyFilters = (overrides: Record<string, any> = {}) => {
    router.get(route('hr.recruitment.job-types.index'), {
      page:     1,
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    if (formMode === 'create') {
      if (!globalSettings?.is_demo) toast.loading(t('Creating job type...'));
      router.post(route('hr.recruitment.job-types.store'), formData, {
        onSuccess: (page) => {
      ...
      if (!globalSettings?.is_demo) toast.loading(t('Updating job type...'));
      router.put(route('hr.recruitment.job-types.update', currentItem.id), formData, {
        onSuccess: (page) => {
          if (!globalSettings?.is_demo) toast.dismiss();
          if (page.props.flash.success) { toast.success(t(page.props.flash.success)); resetForm(); }
          else if (page.props.flash.error) toast.error(t(page.props.flash.error));
  ...
  const handleDeleteConfirm = () => {
    if (!globalSettings?.is_demo) toast.loading(t('Deleting job type...'));
    router.delete(route('hr.recruitment.job-types.destroy', currentItem.id), {
      onSuccess: (page) => {
      ...
      onError: (errors) => {
        ...
        toast.error(typeof errors === 'string'
          ? t(errors)
          : t('Failed to delete job type: {{errors}}', { errors: Object.values(errors).join(', ') }));
      },
    });
  };
  ...
  const handleToggleStatus = (item: any) => {
    ...
    if (!globalSettings?.is_demo) {
      toast.loading(newStatus === 'active' ? t('Activating job type...') : t('Deactivating job type...'));
    }
    router.put(route('hr.recruitment.job-types.toggle-status', item.id), {}, {
      onSuccess: (page) => {
        if (!globalSettings?.is_demo) toast.dismiss();
        if (page.props.flash.success) toast.success(t(page.props.flash.success));
        else if (page.props.flash.error) toast.error(t(page.props.flash.error));
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 sticky top-4">
            <div className="p-6 border-b border-gray-200 dark:border-gray-700">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                {formMode === 'create' ? t('Add New Job Type') : t('Edit Job Type')}
              </h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                {formMode === 'create'
                  ? t('Fill in the details to create a new job type')
                  : t('Update the job type details below')}
              </p>
            </div>
            ...
            <form onSubmit={handleFormSubmit} className="p-6 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder={t('e.g., Full-time, Part-time, Contract')}
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder={t('Brief description of the job type')}
                  rows={3}
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                {(canCreate || canEdit) && (
                  <Button type="submit" className="flex-1">
                    {formMode === 'create' ? t('Add Job Type') : t('Update Job Type')}
                  </Button>
                )}
                {formMode === 'edit' && (
            ...
            <div className="space-y-4">
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <Input
                    type="text"
                    placeholder={t('Search job types...')}
                    value={searchTerm}
            ...
            {(jobTypes?.data || []).length > 0 ? (
              <>
                <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{t('Job Types')}</h3>
                  <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                    {t('Manage job types used in recruitment')}
                  </p>
                </div>
                        ...
                        <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                          ...
                          <td className="px-4 py-4">
                            <div className="flex items-center">
                              <div className="flex-shrink-0 h-10 w-10 rounded-lg flex items-center justify-center bg-violet-100 text-violet-600 dark:bg-violet-900/30 dark:text-violet-400">
                ...
                {/* Pagination */}
                {jobTypes?.total > (jobTypes?.per_page || 10) && (
                  <div className="border-t border-gray-200 dark:border-gray-700">
                    <Pagination
                      from={jobTypes?.from || 0}
                      to={jobTypes?.to || 0}
                      total={jobTypes?.total || 0}
                      links={jobTypes?.links}
                      entityName={t('job types')}
                      onPageChange={(url) => {
              ...
              <div className="p-12 text-center">
                ...
                <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
                  {t('No job types found')}
                </h3>
                <p className="text-gray-500 dark:text-gray-400 mb-6 max-w-sm mx-auto">
                  {isFilterActive
                    ? t('No job types match your search criteria. Try adjusting your filters.')
                    : t('Create job types to categorize your recruitment positions.')}
                </p>
                {!isFilterActive && canCreate && (
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {t('Use the form on the left to add your first job type.')}
                  </p>
                )}
              </div>
            )}
